...
void Setup_ePWM1(void) {
    ...
    EPwm1Regs.AQCTLB.all = ?;      // set ePWM1B on CMPB up
                                        // clear ePWM1B on CMPB down

    EPwm1Regs.TBPRD = ?;            // 1KHz - PWM signal
    EPwm1Regs.CMPA.half.CMPA =  ? ; // 50% duty cycle first
    EPwm1Regs.CMPB  =   ? ;
    EPwm1Regs.PCCTL.bit.CHPFREQ = ?;    // SYSCLKOUT / 64  = 2.34375MHz
    EPwm1Regs.PCCTL.bit.CHPDUTY = ?;    // 50% duty cycle
    EPwm1Regs.PCCTL.bit.CHPEN = ?;      // 1= enable chopper mode
    EPwm1Regs.PCCTL.bit.OSHTWTH = ?;   // 120 *6.67 ns = 800 ns
}
